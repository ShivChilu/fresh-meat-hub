...
import { 
  ...
  LayoutDashboard, LogOut, Menu, X, Phone, MapPin, Eye, Grid3x3
} from 'lucide-react';
...
console.log('Admin Orders - Backend API URL:', API);
                      ...
                      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-gray-600 space-y-1">
                            <p className="font-medium text-gray-900">{order.customerName || order.customer_name || 'Unknown'}</p>
                            ...
                            <p className="flex items-center gap-1">
                              ...
                              {order.address || 'N/A'}, {order.pincode || 'N/A'}
                            </p>
                            <p className="text-xs text-gray-400">{formatDate(order.createdAt || order.created_at)}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                          <div className="text-right">
                            ...
                            <p className="font-bold text-primary text-xl">₹{(order.totalPrice || order.total || 0).toFixed(2)}</p>
                          </div>
            ...
            <div className="space-y-6 mt-4">
              ...
              <div>
                ...
                <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
                  <p><strong>Name:</strong> {selectedOrder.customerName || selectedOrder.customer_name || 'Unknown'}</p>
                  <p><strong>Phone:</strong> {selectedOrder.phone || 'N/A'}</p>
                  <p><strong>Address:</strong> {selectedOrder.address || 'N/A'}</p>
                  <p><strong>Pincode:</strong> {selectedOrder.pincode || 'N/A'}</p>
                </div>
              </div>
                  ...
                  {selectedOrder.items.map((item, index) => (
                    <div key={index} className="flex justify-between items-center py-2 border-b border-gray-100">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">{item.weight || item.unit || 'N/A'} × {item.quantity || 0}</p>
                      </div>
              ...
              <div className="flex justify-between items-center pt-4 border-t">
                ...
                <span className="font-bold text-primary text-xl">₹{(selectedOrder.totalPrice || selectedOrder.total || 0).toFixed(2)}</span>
              </div>
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="font-medium">{selectedOrder.paymentMode || selectedOrder.payment_mode || 'Cash on Delivery'}</p>
                </div>
                <div className="text-right">
              ...
              {/* WhatsApp Button */}
              <a
                href={`https://wa.me/91${selectedOrder.phone}?text=Hi ${selectedOrder.customerName || selectedOrder.customer_name}, your order %23${selectedOrder.id.slice(-8).toUpperCase()} is ${selectedOrder.status.toLowerCase()}.`}
                target="_blank"
